    ...
    const char *text, int len, GdiFont *font, struct nk_color cbg, struct nk_color cfg)
{
    ...
    wsize = MultiByteToWideChar(nk_utf_mode?CP_UTF8:CP_ACP, 0, text, len, NULL, 0);
    wstr = (WCHAR*)_alloca(wsize * sizeof(wchar_t));
    MultiByteToWideChar(nk_utf_mode?CP_UTF8:CP_ACP, 0, text, len, wstr, wsize);
...
void
nk_gdifont_del(nk_gdifont font)
{
    if(!font) return;
    DeleteObject(((GdiFont*)font)->handle);
    DeleteDC(((GdiFont*)font)->dc);
    free(font);
}
    ...
    if (IsClipboardFormatAvailable(CF_UNICODETEXT) && OpenClipboard(NULL))
    {
        ...
        if (mem)
        {
            ...
            if (size)
            {
                ...
                if (wstr) 
                {
                    int utf8size = WideCharToMultiByte(nk_utf_mode?CP_UTF8:CP_ACP, 0, wstr, (int)(size / sizeof(wchar_t)), NULL, 0, NULL, NULL);
                    if (utf8size)
                    {
                        char* utf8 = (char*)malloc(utf8size);
                        if (utf8)
                        {
                            WideCharToMultiByte(nk_utf_mode?CP_UTF8:CP_ACP, 0, wstr, (int)(size / sizeof(wchar_t)), utf8, utf8size, NULL, NULL);
...
nk_gdi_clipboard_copy(nk_handle usr, const char *text, int len)
{
    ...
    {
        int wsize = MultiByteToWideChar(nk_utf_mode?CP_UTF8:CP_ACP, 0, text, len, NULL, 0);
        if (wsize)
        {
...
nk_gdi_init(unsigned int width, unsigned int height, const char *name)
{
    ATOM atom;
    ...
    wnd = CreateWindowEx(exstyle, wc.lpszClassName, name?name:"Nuklear Window",
...
NK_API void
nk_gdi_set_font(nk_gdifont gdifont)
{
    struct nk_user_font *font = &((GdiFont*)gdifont)->nk;
    font->userdata = nk_handle_ptr(gdifont);
    font->height = (float)((GdiFont*)gdifont)->height;
    font->width = nk_gdifont_get_text_width;
...
NK_API void 
nk_gdi_dispatch(struct nk_context *ctx, int *running) {
