...
struct Car {
   double want_v; //wanted speed (Km/h)
   double v;         //correct speed (Km/h)
   double x;         //correct coordinat (m) 
   double t_brake;//time to start braking
   int status;        //action car
} cars[1000];
...
void about() {
                ...
                NK_WINDOW_BORDER))
   {
      ...
      char *about_text = "Программа по моделированию движения автомобилей на автостраде, для ввода параметров моделирования в соотвествующих элементах передвинте ползунок или укажите нужное значение в соответвующих диапазонах. Нажмите на 'Начало' для запуска процесса моделирования движения. Вы можете изменять параметы в процессе. Параметр 'Коэффициент внешних факторов' может содердать такие условия как гололед или состояние автиомобилей, в зависимости от характера фактора Вы можете выставить нужное значение. Укажите величину 'Какую скорость придать автомобилю', чтобы на это значение изменить скорость автомобиля по середине движущийся колонны. если произойдет столкновение модель остановится. для повторного запуска снова нажмите 'Начало'. Для выхода 'Закрыть'. Для возвращения в меню 'Назад'." ;
